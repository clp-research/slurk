...
#   SLURK_OPENVIDU_URL: URL of the OpenVidu server
#   SLURK_OPENVIDU_SECRET: Password for the OpenVidu server, defaults to MY_SECRET
...
if [ -z ${SLURK_OPENVIDU_URL+x} ]; then
    echo 'SLURK_OPENVIDU_URL not defined.';
    exit 2
fi
...
if [ -z ${SLURK_DOCKER+x} ]; then
    ...
    docker run -d \
        ...
        -e SLURK_OPENVIDU_SECRET=${SLURK_OPENVIDU_SECRET:-MY_SECRET} \
