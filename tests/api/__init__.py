...
class RequestOptionsTemplate(Template):
    ...
    def test_request_option_with_id(self, client, request, option):
        ...
        if table_inst is None:
            pytest.skip(f'Depends on tests/api/test_{self.table_name}.py::TestPostValid')
        response = client.options(f'/slurk/api/{self.table_name}/{table_inst.json["id"]}')
        if option == 'OPTIONS':
            assert response.status_code == HTTPStatus.OK
        if response.status_code == HTTPStatus.OK:
            assert option in response.headers['Allow'], HTTPStatus.METHOD_NOT_ALLOWED.description
